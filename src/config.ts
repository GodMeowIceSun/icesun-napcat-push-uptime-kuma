...
export function buildConfigSchema(ctx: NapCatPluginContext): PluginConfigSchema {
    return ctx.NapCatConfig.combine(
        ...
        // 在这里添加你的配置项
        // 推送URL
        ctx.NapCatConfig.text('pushUrl', '推送URL', '', 'https://{domain}/api/push/{token}?status=up&msg=OK&ping='),
        // 心跳间隔（秒）
        ctx.NapCatConfig.number('heartbeatInterval', '心跳间隔（秒）', 60, '推送心跳到UptimeKuma的时间间隔(秒)')
    );
}
